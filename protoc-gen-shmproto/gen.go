...
func (g *Generator) DumpMessage(msg *descriptor.DescriptorProto, currentTAB string) error {
	...
	for _, field := range msg.GetField() {
		if isCommonMessage(field) {
			fmt.Fprintf(buf, "\n%sif(pb.has_%s()){", methodTab, field.GetName())
			fmt.Fprintf(buf, "\n%s    shm_proto::ReadFrom(%s, pb.%s());", methodTab, field.GetName(), field.GetName())
			fmt.Fprintf(buf, "\n%s}", methodTab)
		} else {
			fmt.Fprintf(buf, "\n%sshm_proto::ReadFrom(%s, pb.%s());", methodTab, field.GetName(), field.GetName())
		}
	...
	for _, field := range msg.GetField() {
		if g.isComplextType(field, false) {
			fmt.Fprintf(buf, "\n%sshm_proto::WriteTo(%s, *(pb.mutable_%s()));", methodTab, field.GetName(), field.GetName())
		} else {
			fmt.Fprintf(buf, "\n%spb.set_%s(%s);", methodTab, field.GetName(), field.GetName())
		}
	}
	fmt.Fprintf(buf, "\n%s}\n", fieldTab)
